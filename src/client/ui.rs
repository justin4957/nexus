...
fn draw_status_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    if app.is_scrolled(app.active_channel.as_deref()) {
        spans.push(Span::styled(" ↑ SCROLLED", Style::default().fg(Color::Yellow)));
    }
    ...
    if app.view_mode == ViewMode::ActiveChannel {
         ...
         for (ch_name, content_str, timestamp) in visible_items {
             ...
             for mut line_content in text.lines {
                 line_content.spans.insert(0, Span::styled(format!("#{:<8} │ ", ch_name), Style::default().fg(color)));
...
fn draw_input(f: &mut Frame, app: &App, area: Rect) {
    ...
    let p = Paragraph::new(Line::from(vec![
        Span::styled(format!("#{}", channel_name), Style::default().fg(Color::Cyan)),
    ...
    let cursor_char_idx = input_content[..app.line_editor.cursor_position()].chars().count();
    let _cursor_x = area.x + prefix_len as u16 + cursor_char_idx as u16;
    f.set_cursor(area.x + prefix_len as u16 + cursor_char_idx as u16, area.y);
}